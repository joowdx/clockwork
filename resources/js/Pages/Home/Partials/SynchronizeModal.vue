...
onMounted(() => {
    props.scanners?.forEach(scanner => {
        echo.private(`scanners.${scanner.id}`).listen('TimelogsSynchronization', (event) => {
            ...
            form.value[event.id].wasSuccessful = false
...
onBeforeUnmount(() => {
    props.scanners?.forEach(scanner => echo.leave(`scanners.${scanner.id}`))
})
...
<template>
    <Modal v-model="modelValue" v-model:disableClosing="synchronizing">
            ...
            <p class="flex items-center justify-between font-mono text-sm" v-for="scanner in scanners">
                ...
                <div class="space-x-3">
                    ...
                    <Transition enter-from-class="opacity-0" leave-to-class="opacity-0" class="transition ease-in-out">
                        <span v-if="form[scanner.id].recentlySuccessful" class="text-sm opacity-50">
                            synchronizing...
                        </span>
                    </Transition>

                    <button @click="download(scanner.id)" class="btn btn-xs btn-square btn-primary" :disabled="synchronizing || form[scanner.id].recentlySuccessful">
                        <svg :class="{'animate-spin': form[scanner.id].recentlySuccessful}" class="w-4 h-4 fill-current" xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 512 512">
        ...
        <div class="font-mono text-sm text-warning">
            Click synchronize button then we'll notify you when the process is finished.
        </div>
    </Modal>
</template>
