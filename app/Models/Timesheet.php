...
class Timesheet extends Model
{
    ...
    {
        return $builder->whereHas('export', function ($query) use ($level, $period) {
            $query->when($level, function ($query, $level) {
                match ($level) {
                    'employee' => $query->whereNotNull('details->certification->at'),
                    default => $query->whereNotNull("details->verification->{$level}->at"),
                };
            });

            $query->when($period, function ($query, $period) {
