...
class PulseUserResolver implements ResolvesUsers
{
    /**
     * The resolved users.
     *
    ...
    {
        $this->resolvedUsers = $keys->groupBy(fn ($key) => explode(':', $key)[0])
            ->map(fn ($keys, $guard) => $guard::findMany($keys->map(fn ($key) => explode(':', $key)[1])->toArray()))
            ->flatten();
